...
btn.addEventListener("click", function (event) {
  ...
  axios
    .post("http://16.171.235.235:3000/add-expense", Exp, {
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  function fetchExpenses(page) {
    ...
    axios
      .get(
        `http://16.171.235.235:3000/get-expense?page=${page}&count=${count}`,
        {
          headers: { Authorisation: token },
        }
      )
...
function premiumUserUi() {
 ...
 // message.textContent = "You are a Premium User";
  ...
  document.getElementById("downloadExpenses").onclick = async function (e) {
    ...
    const token = localStorage.getItem("token");
    await axios
      .get("http://16.171.235.235:3000/download-expense", {
        headers: { Authorisation: token },
      })
      .then((response) => {
        console.log(response);
    ...
    axios
      .get("http://16.171.235.235:3000/get-url", {
        headers: { Authorisation: token },
      })
      .then((result) => {
...
function showUser(expense) {
  ...
  delBTN.addEventListener("click", function () {
    axios
      .delete(`http://16.171.235.235:3000/delete-expense/${list.id}`)
      .then(() => {
...
Premium.addEventListener("click", function (event) {
  const token = localStorage.getItem("token");
  axios
    .get("http://16.171.235.235:3000/Premium-Membership", {
      headers: { Authorisation: token },
    })
    .then((response) => {
      ...
      var options = {
        ...
        handler: function (response) {
          axios
            .post(
              "http://16.171.235.235:3000/Transaction-Status",
              {
  ...
  BTN.onclick = async () => {
    ...
    try {
      const LeaderboardArray = await axios.get(
        "http://16.171.235.235:3000/showLeaderBoard",
        { headers: { Authorisation: token } }
      );
